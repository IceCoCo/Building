...
export class remap3D {
    ...
    if (_obj.show == null || typeof (_obj.show) == 'undefined' || _obj.show) {
      ...
      switch (_obj.objType) {
        ...
        case 'cloneObj':
          ...
          break;
        case 'singleBed' :
          _tempObj = _this.addSingleBed (_this, _obj);
          _this.addObject(_tempObj);
          break;
        case 'smokeDetector' :
  ...
  addSmokeDetector (_obj) {
    ...
    var objList = []
    FBXload.load(_obj.url, function (object) {
      // object.scale.multiplyScalar(.1);    // 缩放模型大小
      object.children.forEach(function(item){
        item.name = _obj.name;
        item.uuid = _obj.uuid;
        item.position.x = _obj.x
        item.position.y = _obj.y
        item.position.z = _obj.z
        item.type = _obj.objType
        objList.push(item)
      ...
      if (v.show == null || typeof (v.show) == 'undefined' || v.show) {
        ...
        switch (v.objType) {
          ...
          case 'cloneObj':
            ...
            break;
          case 'singleBed' :
            _tempObj = _this.addSingleBed (_this, v);
            _this.addObject(_tempObj, 'group', group);
            break;
          case 'smokeDetector' :
            _tempObjList = _this.addSmokeDetector(v);
            _tempObjList.forEach((v, i) => {
              _this.addObject(v, 'group', group)
            })
            break;
